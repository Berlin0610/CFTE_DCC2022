...
def listformat_adptive(refframetensor, true_ae_number, num_channel,N_size):    
...
def listformat_adptivenew(refframetensor, true_ae_number, num_channel,N_size):    
   
    current_value=(np.array(refframetensor)+np.array(true_ae_number)).tolist()

    kp_value=current_value[:16] 
    ...
    for channel_size in range(num_channel):
        ...
        latentformat_channel=[]
        for slideformat in range(0,slidelist):
            latentformatslide=kp_value[slideformat*listnum:(slideformat+1)*listnum]
            latentformat_channel.extend([latentformatslide])
        del kp_value[0:length]
        #latentformat=[[latentformat]]
        latentformat.extend([latentformat_channel])
    #print(latentformat)
    latentformat=[latentformat]    
    ...
    ref_idx=current_value[16:17][0]         

    
    return current_value, latentformat,ref_idx